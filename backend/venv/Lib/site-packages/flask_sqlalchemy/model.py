...
class Model:
    ...
    To define models, subclass :attr:`Base <.SQLAlchemy.Model>`, not this. To
    customize ``Base``, subclass this and pass it as ``model_class`` to
    :class:`.SQLAlchemy`. To customize ``Base`` at the metaclass level, pass an
    ...
    """A SQLAlchemy query for a model. Equivalent to ``session = SessionLocal()
# ...do stuff...
session.add(...)
session.commit()
session.close()
